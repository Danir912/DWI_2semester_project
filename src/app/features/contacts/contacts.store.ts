...
export const ContactsStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    categories: computed(() =>
      Array.from(new Set(store.contacts().map((contact) => contact.category))).sort(),
    ),
    selectedContact: computed(
      () => store.contacts().find((contact) => contact.id === store.selectedId()) ?? null,
    ),
    reminders: computed(() =>
      store
        .contacts()
    ...
    pendingRemindersCount: computed(() =>
      store
        ...
        .reduce(
          (sum, contact) =>
            sum + contact.reminders.filter((reminder) => !reminder.completed).length,
          0,
        ),
    ),
    ...
    const persistContact = (contact: Contact): void => {
      ...
      patchState(store, ({contacts}) => ({
        contacts: contacts.map((item) =>
          item.id === normalizedContact.id ? normalizedContact : item,
        ),
      }));

      api.updateContact(normalizedContact).subscribe({
    ...
    return {
      ...
      setFilters(filters: Partial<ContactFilters>): void {
        patchState(store, (state) => {
          ...
          const selectedVisible = visibleContacts.some(
            (contact) => contact.id === state.selectedId,
          );
        ...
        pipe(
          ...
          switchMap((payload) =>
            api.createContact(payload).pipe(
              tap((contact) =>
                patchState(store, ({contacts}) => ({
                  contacts: [normalizeContact(contact), ...contacts],
          ...
          switchMap((contact) =>
            api.updateContact(contact).pipe(
              tap((updated) =>
                patchState(store, ({contacts}) => ({
                  contacts: contacts.map((item) =>
                    item.id === updated.id ? normalizeContact(updated) : item,
                  ),
                })),
              ),
            ),
              ...
              tap(() =>
                patchState(store, ({contacts, trash}) => {
                  ...
                  return {
                    ...
                    trash: nextTrash,
                    selectedId: nextContacts[0]?.id ?? null,
                  };
                }),
              ),
            ),
      ...
      restoreContact: rxMethod<ArchivedContact>(
        pipe(
          switchMap((archived) => {
            const alreadyRestored = store
              .contacts()
              .some((contact) => contact.id === archived.contact.id);
            const nextTrash = store
              .trash()
              .filter((item) => item.contact.id !== archived.contact.id);
            ...
            return api.restoreContact(normalizeContact(archived.contact)).pipe(
              tap((contact) =>
                patchState(store, ({contacts}) => ({
                  contacts: contacts.some((item) => item.id === contact.id)
                    ? contacts
                    : [normalizeContact(contact), ...contacts],
                  selectedId: contact.id,
                })),
              ),
            );
      ...
      addReminder(contactId: string, reminder: Reminder): void {
        ...
        persistContact({
          ...contact,
          nextContactAt: resolveNextContactAt(contact.nextContactAt, reminder.dueDate),
      ...
      importContacts(contacts: Contact[]): void {
        ...
        const requests = normalized.map((contact) =>
          api.upsertContact(contact, existingIds.has(contact.id)),
        );
          ...
          next: (savedContacts) =>
            patchState(store, ({contacts: currentContacts}) => ({
              contacts: currentContacts.map((contact) => {
                const savedContact = savedContacts.find((saved) => saved.id === contact.id);

                return savedContact ? normalizeContact(savedContact) : contact;
              }),
              loading: false,
            })),
          error: () =>
            patchState(store, {
              loading: false,
              error: 'Импортированные контакты не синхронизированы',
            }),
        });
      },
    };
